    ...
    import './dashBoaVista.css';
    import { useState } from 'react';
    import firebase from '../../services/firebaseConnection';
    

    export default function DashBoaVista () {
        const listRef = firebase.firestore().collection('boavista').where('Tipo', '==', 'Implementação');
        const listRef3 = firebase.firestore().collection('boavista').where('Tipo', '==', 'Entendimento');
        const listRef4 = firebase.firestore().collection('boavista').where('Tipo', '==', 'Melhoria');
        const listRef2 = firebase.firestore().collection('boavista').where('Projeto', '==', 'Boa Vista');
        const listRef8 = firebase.firestore().collection('boavista').where('Ambiente', '==', 'Local');
        const listRef9 = firebase.firestore().collection('boavista').where('Ambiente', '==', 'Cresol');
        const listRef10 = firebase.firestore().collection('boavista').where('Ambiente', '==', 'Homologação');
        const listRef5 = firebase.firestore().collection('indicadorboavista').where('Cenário', '>', 0);
        const listRef6 = firebase.firestore().collection('indicadorboavista').where('Res', '>=', 25);
        const listRef7 = firebase.firestore().collection('indicadorboavista').where('Res', '<', 25);
